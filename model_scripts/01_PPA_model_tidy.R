...
# combined demographic rates  the Inital state from 2020. 
# This script to run this script use the the R scripts 
# "04_run_tidy_combined_rates_elevated_groundwater.R" located in the folder 
# "model_scripts".
...
main_cohorts_tidy = function(
 ...
 all_cohorts_out<- all_cohorts_out %>%  mutate(sp_name = case_when(sp == 2 ~ "Acer campestre",
                             sp == 5 ~ "Acer platanoides",
                             sp == 1 ~ "Acer pseudoplatanus",
                             sp == 4 ~ "Carpilus betulus",
                             sp == 3 ~ "Fraxinus exclesior",
                             sp == 6 ~ "Quercus robur",
                             sp == 7 ~ "Ulmus spp.",
                             sp == 8 ~ "Tilia spp.",
...
prepare.initdata = function(initdata, spdata, dnot, deltaT, mincohortN, PA) {
  ...
  if(is_tibble(initdata)) {
    ...
    data = data %>% mutate(sp = as.integer(factor(sp, 
                                                  levels = c("BAH", "FAH","GES", "HBU", "SAH", "SEI", "UL", "WLI"))))
...
all_cohorts_out$sp <- as.character(all_cohorts_out$sp)
  ...
  p4 = all_cohorts_out %>%
    ...
    mutate(sp = case_when(sp == 2 ~ "Acer campestre",
                          sp == 5 ~ "Acer platanoides",
                          sp == 1 ~ "Acer pseudoplatanus",
    ...
    scale_color_manual(values = c("Acer platanoides" = "#228B22",
                                  "Acer pseudoplatanus" = "#00FF00",
                                  "Carpinus betulus" = "#FFA500",
                                  "Fraxinus excelsior" = "blue",
                                  "Quercus robur" = "red",
                                  "Ulmus spp." = "#17becf",
                                  "Tilia spp." = "grey",
                                  "Acer campestre" = "#808000"))+
    ...
    theme(
      ...
      axis.text.y = element_text(margin = margin(r = 2))  # Adjust margin for y-axis text# Add ticks to the axes  
    )+
