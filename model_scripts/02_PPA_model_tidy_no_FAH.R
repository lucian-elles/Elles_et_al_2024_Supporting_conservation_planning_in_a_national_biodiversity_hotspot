...
# demographic rates from 2013 - 2020 and the Inital state from 2016. This script
# excludes Acer campestre (FAH). To run this script use the the R scripts 
...
main_cohorts_tidy = function(
  ...
  # name trees 
  all_cohorts_out<- all_cohorts_out %>%  mutate(sp_name = case_when(sp == 4 ~ "Acer platanoides",
                                                                    sp == 1 ~ "Acer pseudoplatanus",
                                                                    sp == 3 ~ "Carpinus betulus",
...
prepare.initdata = function(initdata, spdata, dnot, deltaT, mincohortN, PA) {
  ...
  if(is_tibble(initdata)) {
    ...
    data = data %>% mutate(sp = as.integer(factor(sp, 
                                                  levels = c("BAH","GES", "HBU", "SAH", "SEI", "UL", "WLI"))))
